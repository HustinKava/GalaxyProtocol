...
const GalaxyUniversity = () => {
  ...
  const handler = (filterType) => {
    ...
    if (articleFilters.indexOf(filterWord) !== -1) {
      ...
      const indexOfWordInstance = articleFilters.findIndex(
  ...
  return (
    <div>
      <Header />
      <GradientBar handler={handler} articleFilters={articleFilters} />
      <ArticleGallery articleFilters={articleFilters} />
      <Socials />
      <Partners />
      <VideosCarousel />
    </div>
